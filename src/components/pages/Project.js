                ...
                <div className={styles.project_details}>
                    <Container customClass="column">
                        ...
                        <div className={styles.details_container}>
                            ...
                            {!showProjectForm ? (
                                <div className={styles.project_info}>
                                    ...
                                    <p>
                                        <span>Orçamento Inicial: </span>R$ {parseFloat(project.budget).toLocaleString('pt-BR')}
                                    </p>
                                    <p>
                                        <span>Orçamento Utilizado: </span>R$ {project.cost.toLocaleString('pt-BR')}
                                    </p>
                                    <p><span>Orçamento Atual: </span>R$ {remainingBudget.toLocaleString('pt-BR')}</p>
                                </div>
                            ) : (
                                <div className={styles.project_info}>
                                    <ProjectFrom 
                                    handleSubmit={editPost} btnText='Salvar' 
                                    projectData={project} />
                                </div>
                            )}
                        </div>
                        ...
                        {message && <Message type={type} msg={deleteMessage}/>}
                            {services.length > 0 &&
                            services
                            .filter((service) => service.name.toLowerCase().includes(filter.toLowerCase()) || filter === "")
                            .map((service) =>(
                                <ServiceCard
                                    id={service.id}
                                    name={service.name}
